...
class PDFConverterApp:
    def __init__(self, root):
        ...
        self.dpi_var = tk.IntVar(value=300)          # 传递给MinerU
        self.lang_var = tk.StringVar(value="中英文")  # 传递给MinerU
        ...
        self.use_new_pypdf2 = USE_NEW_PYPDF2
    ...
    def setup_ui(self):
        ...
        tk.Label(title_frame, text="📚 木子 PDF 转换器", font=("微软雅黑", 18, "bold"),
                 fg="#0A192F").pack(side=tk.LEFT)  # 粉色可爱
    ...
    def check_pdf_encryption(self, pdf_path):
        ...
        try:
            with open(pdf_path, 'rb') as f:
                reader = PdfReader(f)
                is_encrypted = reader.is_encrypted if self.use_new_pypdf2 else reader.isEncrypted
                if is_encrypted:
                    cached = self.password_cache.get(pdf_path)
                    if cached:
                        try:
                            if self.use_new_pypdf2:
                ...
                with open(pdf_path, 'rb') as f:
                    ...
                    if password:
                        if self.use_new_pypdf2:
                            ...
                            reader.decrypt(password)
                    total_pages = len(reader.pages) if self.use_new_pypdf2 else reader.getNumPages()
